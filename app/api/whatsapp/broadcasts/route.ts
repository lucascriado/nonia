...
type Payload = { audience?: string; message?: string; filters?: Record<string, string>; preview?: boolean };
...
export async function POST(request: Request) {
  try {
    ...
    const lista = await envio.candidatos(audience, params, org);
      ...
      for (const alvo of alvos) {
        await db.query(
          `INSERT INTO whatsapp_broadcast_recipients
           ...
           VALUES ($1, $2, $3, $4, $5, $6)`,
          {
            bind: [broadcastId, org, alvo.id, alvo.name, alvo.phone, alvo.chatId ? "pending" : "skipped"],
            transaction,
          },
        );
      }
